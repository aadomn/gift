/****************************************************************************
* Constant-time ARM assembly implementation of the GIFT-128 block cipher.
*
...
gift128_encrypt_block:
    ...
    rev r6, r6
    rev r7, r7
    // rearrange half words before SWAPMOVE routines
    uxth r8, r5
    uxth r9, r7
    orr r9, r9, r8, lsl #16
    uxth r8, r5, ror #16
    ...
    // SWAPMOVE for a row-wise bitsliced representation
    movw r2, #0x0a0a
    movt r2, #0x0a0a
    eor r3, r9, r9, lsr #3
    and r3, r3, r2
    ...
    // ------------------ GIFTb-CORE ROUTINE ------------------
    mov r3, r2 //0x000f000f for HALF_ROR
    movw r2, #0x1111
    ...
    eor r12, r12, r3
    eor r12, r12, r3, lsl #3 //SWAPMOVE(r12, r12, r2, 3)
    // rearrange half words after SWAPMOVE routines
    lsr r4, r12, #16
    lsr r5, r11, #16
...
/*****************************************************************************
* Fully unrolled ARM assembly implementation of the GIFT-128 block cipher in 
* CTR mode. The 128-bit IV is built from a 32-bit counter and a 96-bit nonce 
* in the following way: IV = ctr||nonce.
...
gift128_encrypt_ctr:
    ...
    // ------------------ PACKING ------------------ 
    // performs packing for words that do not depends on counter
    // load the nonce
    ldr.w r5, [r1, #4]
    ldr.w r7, [r1, #12]   
    // endianness
    ...
    eor r10, r10, r3
    eor r9, r9, r3, lsl #4 //SWAPMOVE(r9, r10, r2, 4)
    //stock on the stack for the next blocks to encrypt
    push {r9-r10}
    // r2 already contains 0a0a0a0a after the unpacking routine
    movw r2, #0x0a0a
    movt r2, #0x0a0a

gift_ctr_encrypt_block:
    // ------------------ PACKING ------------------
    // performs packing for words that depend on counter 
    ldr.w r4, [r1] //32-bit counter
    ldr.w r6, [r1, #8]
    ldrd r9, r10, [sp]
    // endianness
    rev r4, r4
    ...
    str.w r12, [sp] //r2 now points to the next ptext block

    // ------------------ UPDATE COUNTERS AND ADRESSES ------------------
    //decrement 'ptext_len' by 16 (block size)
    ldr.w r4, [sp, #4]
    subs r4, #16
    ble gift_ctr_exit
    str.w r4, [sp, #4]
    sub sp, #8
    //increment 'p.ctr' by 1
    ldr r4, [r1]
    add r4, #1
    str r4, [r1]
...
/*****************************************************************************
* Fully unrolled assembly implementation of the GIFTb-128 block cipher in 
* CTR mode. The 128-bit IV is built from a 32-bit counter and a 96-bit nonce 
* in the following way: IV = ctr||nonce.
* This functions take as input an IV, plaintext blocks and precomputed rkeys 
* and increments the counter for each block to encrypt.
...
giftb_ctr_encrypt_block:
    ...
    eor r11, r11, r6
    eor r12, r12, r7
    stmia r0!, {r9-r12}  //r0 now points to the next ctext block
    str.w r3, [sp] //now points to the next ptext block
